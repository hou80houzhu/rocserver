 ...
 * @author hou80houzhu
 */
public class XLoginService extends BaseSocketService {

    @Override
    public void doService() throws Exception {
        ResultSet result_one=this.getResultSet();
        result_one.setCode(ResultSet.RESULT_SUCCESS).setService(ClientServiceType.SERVICE_LOGIN).setData(this.getConnect().id()).setRoutName(BaseSocketRouter.ROUT_NAME_SIMPLE).setRoutType(BaseSocketRouter.ROUT_SELF);
        ConnectGroup.getConnectGroup().addConnect(null, connect.id());
